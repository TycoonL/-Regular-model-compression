...
if __name__ == '__main__':
    ...
    #weight_file = './results/' + save_name + '.h5'  #322
    weight_file = './results2/compressed_vgg_weights.h5' #+ save_name + '_weights.h5'
    # decode
    weights = decode_weights(weight_file)
    for i in range(len(model.layers)):
        if model.layers[i].name in weights:
            weight = [w for w in weights[model.layers[i].name]]
            #weight=weights[model.layers[i].name][model.layers[i].name]['kernel:0'][:]
            #print(weight)
